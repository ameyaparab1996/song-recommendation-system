...
def display_recommendations(spotify_df, positive_prompt):
    ...
    with st.form(key='playlist_form'):
        ...
        if st.form_submit_button(label='Create Playlist', on_click=spotify_redirect( st.session_state.sp_oauth,  st.session_state.redirected_url, list(spotify_df.loc[spotify_df['include'] == True, 'track_uri']), st.session_state.username, st.session_state.playlist_name, positive_prompt)):
    #if create_button:
